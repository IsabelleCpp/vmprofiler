...
profiler_t jmp = {
    ...
    [](zydis_reg_t& vip, zydis_reg_t& vsp,
      ...
      if (xchg != instrs.end()) {
        ...
        zydis_reg_t write_dep = xchg->operands[0].reg.value != vsp
                                    ? xchg->operands[0].reg.value
                                    : xchg->operands[1].reg.value;

        // update VIP... VSP becomes VIP... with the XCHG...
        vip = xchg->operands[0].reg.value != vsp
                  ? xchg->operands[1].reg.value
                  : xchg->operands[0].reg.value;
        ...
        const auto mov_reg_write_dep = std::find_if(
            xchg, instrs.end(), [&](const emu_instr_t& instr) -> bool {
              ...
              return i.mnemonic == ZYDIS_MNEMONIC_MOV &&
                     instr.operands[0].type == ZYDIS_OPERAND_TYPE_REGISTER &&
                     instr.operands[1].type == ZYDIS_OPERAND_TYPE_REGISTER &&
                     instr.operands[1].reg.value == write_dep;
            });

        if (mov_reg_write_dep == instrs.end()) return {};

        vsp = mov_reg_write_dep->operands[0].reg.value;
      } else {
        ...
        const auto mov_vip_reg = std::find_if(
            ...
            [&](const emu_instr_t& instr) -> bool {
              ...
              return i.mnemonic == ZYDIS_MNEMONIC_MOV &&
                     instr.operands[0].type == ZYDIS_OPERAND_TYPE_REGISTER &&
                     instr.operands[1].type == ZYDIS_OPERAND_TYPE_REGISTER &&
                     instr.operands[1].reg.value ==
                         mov_reg_deref_vsp->operands[0].reg.value;
            });

        if (mov_vip_reg == instrs.end()) return {};

        vip = mov_vip_reg->operands[0].reg.value;
        ...
        if (mov_reg_vsp != instrs.end())
          vsp = mov_reg_vsp->operands[0].reg.value;
      }
